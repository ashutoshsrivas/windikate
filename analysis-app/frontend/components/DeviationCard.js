...
export default function DeviationCard({ analysisId, deviation, onChange, questions = [] }) {
    ...
    return (
        <article className={`rounded-2xl border bg-surface p-5 sev-${effective}`} style={{ borderColor: 'var(--tw-shadow,initial)' }}>
            <header className="flex items-start justify-between gap-3">
                ...
                {!editing && (
                    <button onClick={() => setEditing(true)} className="text-xs text-ink2-muted hover:text-ink2 px-2.5 py-1 rounded-md hover:bg-surface-raised border border-edge">
                        <i className="bi bi-pencil mr-1" />Edit
                    </button>
                )}
            </header>

            <p className="mt-3 text-sm text-ink2-muted leading-relaxed">{deviation.description}</p>

            <div className="mt-3 flex flex-wrap items-center gap-3 text-[12px] text-ink2-muted">
                ...
                {deviation.benchmark_value && <span className="text-ink2-faint">target {deviation.benchmark_value}</span>}
            </div>

            {deviation.analyst_citation && !editing && (
                <div className="mt-3 p-3 rounded-lg bg-surface-raised border border-edge text-sm text-ink2-muted">
                    <div className="text-[10px] font-mono uppercase tracking-wider text-ink2-faint mb-1">Analyst note</div>
            ...
            {questions.length > 0 && (
                <ul className="mt-4 space-y-1.5 text-sm">
                    {questions.map(q => (
                        <li key={q.id} className="flex items-start gap-2 text-ink2-muted">
            ...
            {editing && (
                <div className="mt-4 p-4 rounded-xl bg-black/30 border border-edge space-y-3">
                    <div>
                        <label className="block text-[11px] font-mono uppercase tracking-wider text-ink2-faint mb-1.5">Severity (OK standard)</label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <input value={draft.benchmark_label} onChange={e => setDraft(d => ({ ...d, benchmark_label: e.target.value }))}
                            ...
                            className="px-3 py-2 rounded-lg bg-surface-raised border border-edge focus:border-brand-500 focus:outline-none text-sm" />
                    ...
                    <div className="flex justify-end gap-2">
                        <button onClick={() => setEditing(false)} className="text-sm text-ink2-muted hover:text-ink2 px-3 py-1.5 rounded-md hover:bg-surface-raised">Cancel</button>
                        <button onClick={save} disabled={busy} className="inline-flex items-center gap-1.5 bg-brand-600 hover:bg-brand-500 disabled:opacity-50 text-ink2 text-sm px-4 py-1.5 rounded-md">
                            {busy ? 'Saving…' : 'Save'}
                        </button>
                    </div>
                </div>
