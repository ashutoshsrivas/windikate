...
-- =====================================================================
-- Adds:
--   · role + AI model allowlist + spend tracking to users
--   · global settings table (KV with JSON values)
--   · usage_events table (one row per AI call, with cost in cents)
--   · seeds the global "default_model" + "bedrock_enabled" rows
--   · promotes demo@windikate.com → admin so we can sign in to /admin
-- =====================================================================
...
INSERT INTO settings (key_name, value) VALUES
    ('default_model',         JSON_QUOTE('us.anthropic.claude-3-5-haiku-20241022-v1:0')),
    ('bedrock_enabled',       CAST('true' AS JSON)),
    ('monthly_cap_cents',     CAST('5000' AS JSON));  -- $50 / month soft cap

CREATE TABLE usage_events (
    id                BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id           BIGINT UNSIGNED NULL,
    analysis_id       BIGINT UNSIGNED NULL,
    service           VARCHAR(40) NOT NULL,       -- e.g. 'questionGenerator'
...
-- Bootstrap: make the existing demo account an admin so we can sign in
