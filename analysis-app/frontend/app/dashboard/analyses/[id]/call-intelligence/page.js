...
export default function CallIntelligenceTab() {
    return (
        <div className="space-y-8">
            <div className="rounded-3xl border border-brand-500/30 bg-gradient-to-br from-brand-500/10 via-surface to-surface p-10 sm:p-14 text-center relative overflow-hidden">
                <div className="absolute inset-0 bg-grid opacity-[0.05]" />
                <div className="absolute -top-20 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full bg-brand-500/20 blur-3xl" />
                <div className="relative">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-brand-500/40 bg-brand-500/10 text-brand-500 text-xs font-mono uppercase tracking-wider mb-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {FEATURES.map(f => (
                    <div key={f.title} className="rounded-2xl border border-edge bg-surface p-6 hover:border-brand-500/30 transition-colors">
                        <div className="w-11 h-11 rounded-xl bg-brand-500/10 text-brand-500 flex items-center justify-center mb-4"><i className={`bi ${f.icon} text-lg`} /></div>
