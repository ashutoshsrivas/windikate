    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-8">
            <div className="space-y-8">
                ...
                <section>
                    <h2 className="text-sm font-semibold uppercase tracking-wider text-ink2-muted mb-3">Extracted financial schema</h2>
                ...
                {/* Deviations */}
                <section>
                    <h2 className="text-sm font-semibold uppercase tracking-wider text-ink2-muted mb-3">Deviations · benchmarked</h2>
            ...
            <aside className="space-y-3">
                <h2 className="text-sm font-semibold uppercase tracking-wider text-ink2-muted">Competitive landscape</h2>
                        ...
                        <div key={c.id} className="rounded-xl border border-edge bg-surface p-4">
                            <div className="flex items-center justify-between gap-3">
                                <div className="min-w-0">
                                    ...
                                    <div className="text-[11px] font-mono uppercase tracking-wider text-ink2-faint">{c.relation}</div>
                                </div>
                                <a href={c.website} target="_blank" rel="noreferrer" className="text-xs text-ink2-faint hover:text-ink2"><i className="bi bi-box-arrow-up-right" /></a>
                            </div>
                            <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
                                <div className="rounded-md bg-surface-raised px-2 py-1.5"><div className="text-ink2-faint">Funding</div><div className="font-medium">${(c.funding_usd / 1e6).toFixed(1)}M</div></div>
                                <div className="rounded-md bg-surface-raised px-2 py-1.5"><div className="text-ink2-faint">Traffic / mo</div><div className="font-medium">{(c.monthly_traffic / 1000).toFixed(0)}K</div></div>
                            </div>
                            {c.features && (
