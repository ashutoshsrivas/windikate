    ...
    return (
        <main className="min-h-screen flex items-start justify-center px-6 py-12">
            <div className="w-full max-w-3xl">
                ...
                {step === 1 && (
                    <section className="mt-8 rounded-3xl border border-white/10 bg-gradient-to-br from-ink-800/80 to-ink-900/80 p-8 sm:p-10">
                        ...
                        <p className="mt-2 text-white/55">A few quick choices so every report is calibrated to how you actually invest.</p>

                        <div className="mt-8 space-y-7">
                            <Field label="What would Windikate call you?">
                                <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Riya"
                                    className="w-full sm:w-80 px-4 py-3 rounded-xl bg-white/5 border border-white/15 focus:border-brand-500 focus:outline-none" />
                            </Field>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                                    {ROLES.map(r => (
                                        <button key={r.value} type="button" onClick={() => setRole(r.value)}
                                            className={`flex flex-col items-start gap-2 px-4 py-4 rounded-xl border transition-all ${role === r.value ? 'border-brand-500 bg-brand-600/15' : 'border-white/15 bg-white/5 hover:bg-white/10'}`}>
                                            <i className={`bi ${r.icon} text-xl ${role === r.value ? 'text-brand-300' : 'text-white/55'}`} />
                                    ...
                                    {FOCUS.map(f => {
                                        ...
                                        return (
                                            <button key={f.value} type="button" onClick={() => toggleFocus(f.value)}
                                                className={`px-3.5 py-2 rounded-full text-sm border transition-colors ${active ? 'bg-brand-600/20 border-brand-500 text-white' : 'bg-white/5 border-white/15 text-white/70 hover:bg-white/10'}`}>
                            ...
                            <div className="pt-2 flex justify-end">
                                <button onClick={continueToStep2} disabled={saving}
                                    className="inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-500 disabled:opacity-50 text-white font-medium px-6 py-3 rounded-xl shadow-glow transition-all">
                ...
                {step === 2 && (
                    <section className="mt-8 rounded-3xl border border-white/10 bg-gradient-to-br from-ink-800/80 to-ink-900/80 p-8 sm:p-10">
                        ...
                        <p className="mt-2 text-white/55">These thresholds let the deviation engine compare against <em>your</em> standards, not just industry medians.</p>

                        <div className="mt-8 space-y-8">
                            <Field label="Typical pre-seed ARR range (₹ Cr)">
                                <div className="flex items-center gap-3">
                                    <input value={arrMin} onChange={e => setArrMin(e.target.value)} type="number" min="0" step="0.1" placeholder="min" className="w-32 px-4 py-3 rounded-xl bg-white/5 border border-white/15 focus:border-brand-500 focus:outline-none" />
                                    <span className="text-white/40">to</span>
                                    <input value={arrMax} onChange={e => setArrMax(e.target.value)} type="number" min="0" step="0.1" placeholder="max" className="w-32 px-4 py-3 rounded-xl bg-white/5 border border-white/15 focus:border-brand-500 focus:outline-none" />
                                </div>
                                <p className="text-xs text-white/40 mt-2">Stored in INR for benchmark comparison.</p>
                            </Field>
                            ...
                            <Field label={`Acceptable CAC : LTV ratio · 1 : ${ratio}`} hint="Drag to set your floor">
                                ...
                                <div className="flex justify-between text-[11px] font-mono text-white/40 w-full sm:w-96 mt-1">
                            ...
                            <div className="pt-2 flex flex-col-reverse sm:flex-row justify-end gap-3">
                                <button onClick={() => finish(false)} disabled={saving}
                                    className="inline-flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/15 text-white font-medium px-6 py-3 rounded-xl transition-all">Skip for now</button>
                                <button onClick={() => finish(true)} disabled={saving}
                                    className="inline-flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-500 disabled:opacity-50 text-white font-medium px-6 py-3 rounded-xl shadow-glow transition-all">
...
function Pill({ active, done, num, label }) {
    return (
        <div className={`flex items-center gap-2 ${active ? 'text-white' : 'text-white/40'}`}>
            <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold border ${done ? 'bg-emerald-500 border-emerald-400 text-white' : active ? 'bg-brand-600 border-brand-500' : 'bg-white/5 border-white/15'}`}>
    ...
    return (
        <div>
            <div className="flex items-baseline justify-between mb-2.5">
                ...
                {hint && <span className="text-[11px] font-mono text-white/40 uppercase tracking-wider">{hint}</span>}
            </div>
            {children}
        </div>
    );
